-- Posts
	...
	BEGIN
		...
		SELECT pd.*,posts.*,ad.username,ad.display_name,ad.admin_id,ad.email
			...
			@IF !empty(:categories) 
			THEN 
				,(SELECT CONCAT('[', GROUP_CONCAT('{"taxonomy_item_id":"', categories.taxonomy_item_id, '","name":"' , td.name, '","slug":"' , td.slug, '"}'), ']')
					...
					LEFT JOIN post_to_taxonomy_item pt ON (categories.taxonomy_item_id = pt.taxonomy_item_id AND pt.post_id = posts.post_id)  
