-- Orders
	...
	BEGIN
		...
		DELETE FROM order_recurring_transaction WHERE order_subscription_plan_id IN (SELECT order_subscription_plan_id FROM order_recurring WHERE order_id IN (:order_id));
