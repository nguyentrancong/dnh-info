...
.settings input[type="text"]|value = <?php 
	$_setting = '@@__name:\[(.*)\]__@@';
	echo $_POST['gravatar'][$_setting]  ?? Vvveb\get_setting('gravatar', $_setting, null) ?? '@@__value__@@';
...
.settings input[type="radio"]|addNewAttribute = <?php 
	...
	if (isset($_POST['gravatar'][$_setting]) && ($_POST['gravatar'][$_setting] == $_value) ||
		(Vvveb\get_setting('gravatar',$_setting, null) == $_value)  ||
		 '@@__checked__@@') { 
			echo 'checked';
	}
?>
...
.settings textarea = <?php 
	$_setting = '@@__name:\[(.*)\]__@@';
	echo $_POST['gravatar'][$_setting] ?? Vvveb\get_setting('gravatar',$_setting, null) '@@__innerHTML__@@';
?>
